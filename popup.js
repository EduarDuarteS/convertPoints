  ...
  cognitoUser.authenticateUser(authenticationDetails, {
    onSuccess: (session) => {
      ...
      const accessTokenExpiration =
        session.getAccessToken().getExpiration() * 1000; // Convertir a milisegundos
...
function refreshAccessToken(email) {
  ...
  if (!storedRefreshToken) {
    ...
    console.log(
      "Refresh token no encontrado. Redirigiendo al inicio de sesión."
    );
  ...
  const cognitoUser = new AmazonCognitoIdentity.CognitoUser(userData);
  const refreshCognitoToken = new AmazonCognitoIdentity.CognitoRefreshToken({
...
function addAccount() {
  ...
  if (encryptedAccessToken) {
    // Desencriptar el token de acceso utilizando la clave de encriptación
    decryptedAccessToken = CryptoJS.AES.decrypt(
      encryptedAccessToken,
      "encryption-key"
    ).toString(CryptoJS.enc.Utf8);
    if (!decryptedAccessToken) {
  ...
  fetch(
    "https://qpfg5gn4rk.execute-api.us-east-2.amazonaws.com/pruebas/addCtaBank",
    requestOptions
  )
